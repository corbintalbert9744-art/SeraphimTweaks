...
const DashboardPreview = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setStats(prev => ({
        totalUsers: prev.totalUsers + (Math.random() > 0.6 ? 1 : 0),
        activeNow: Math.max(600, Math.min(650, prev.activeNow + (Math.random() > 0.5 ? 2 : -2))),
        totalChange: Math.max(12.0, Math.min(12.8, prev.totalChange + (Math.random() > 0.5 ? 0.1 : -0.1))),
        activeChange: Math.max(8.0, Math.min(8.5, prev.activeChange + (Math.random() > 0.5 ? 0.1 : -0.1))),
        barHeights: prev.barHeights.map(h => Math.max(30, Math.min(100, h + (Math.random() > 0.5 ? 3 : -3))))
      }));
    }, 200);
    ...
    <section className="py-20 px-6">
      <div className="max-w-6xl mx-auto">
        <div className="relative animate-in fade-in zoom-in duration-1000 delay-500">
          ...
          <div className={`animate-float bg-[#0f0f0f] backdrop-blur border border-[#1a1a1a] rounded-2xl p-8 shadow-2xl shadow-yellow-500/10 max-w-4xl mx-auto ${clickableStyles}`}>
            ...
            <div className="grid grid-cols-2 gap-8 mb-12 max-w-md mx-auto">
              {[
                { label: 'Total Users', value: stats.totalUsers.toLocaleString(), change: `+${stats.totalChange.toFixed(1)}%` },
                { label: 'Active Now', value: stats.activeNow.toLocaleString(), change: `+${stats.activeChange.toFixed(1)}%` },
              ].map((stat, i) => (
                <div key={i} className={`bg-[#141414]/50 rounded-xl p-6 ${clickableStyles}`}>
            ...
            <div className="h-48 bg-[#141414]/30 rounded-xl flex items-end justify-around p-6">
              {stats.barHeights.map((h, i) => (
                <div key={i} className="w-8 bg-gradient-to-t from-yellow-500 to-amber-500 rounded-t opacity-80 hover:opacity-100 transition-all duration-200 hover:shadow-[0_0_15px_rgba(234,179,8,0.5)]" style={{ height: `${h}%` }} />
              ))}
            </div>
          </div>
        </div>
...
const PerformanceStats = () => {
  const [stats, setStats] = useState({
    fps: { before: 144, after: 280 },
    delay: { before: 12.5, after: 4.2 },
    ping: { before: 45, after: 22 }
  });

  useEffect(() => {
    const interval = setInterval(() => {
      setStats(prev => {
        const nextFpsBefore = Math.max(140, Math.min(150, prev.fps.before + (Math.random() > 0.5 ? 0.5 : -0.5)));
        const nextFpsAfter = Math.max(270, Math.min(295, prev.fps.after + (Math.random() > 0.5 ? 2 : -2)));
        
        const nextDelayBefore = Math.max(12, Math.min(13, prev.delay.before + (Math.random() > 0.5 ? 0.05 : -0.05)));
        const nextDelayAfter = Math.max(3.8, Math.min(4.5, prev.delay.after + (Math.random() > 0.5 ? 0.05 : -0.05)));

        const nextPingBefore = Math.max(40, Math.min(50, prev.ping.before + (Math.random() > 0.5 ? 0.5 : -0.5)));
        const nextPingAfter = Math.max(18, Math.min(25, prev.ping.after + (Math.random() > 0.5 ? 0.5 : -0.5)));

        return {
          fps: { before: nextFpsBefore, after: Math.max(nextFpsBefore + 50, nextFpsAfter) },
          delay: { before: nextDelayBefore, after: Math.min(nextDelayBefore - 5, nextDelayAfter) },
          ping: { before: nextPingBefore, after: Math.min(nextPingBefore - 10, nextPingAfter) }
        };
      });
    }, 200);
    return () => clearInterval(interval);
  ...
  const Card = ({ title, before, after, unit, isLowerBetter = false }: any) => {
    ...
    return (
      <div className="p-6 rounded-2xl bg-[#0f0f0f] border border-[#1a1a1a] hover:border-yellow-500/50 transition-all hover:outline hover:outline-1 hover:outline-yellow-500/50 hover:shadow-[0_0_20px_rgba(234,179,8,0.15)] group">
        <h3 className="text-neutral-400 text-sm font-semibold mb-4 uppercase tracking-wider">{title}</h3>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-xs text-neutral-500 mb-1">Before</p>
            <p className="text-xl font-bold text-neutral-400">{before.toFixed(title === 'Input Delay' ? 1 : 0)}{unit}</p>
          </div>
          <div>
            <p className="text-xs text-yellow-500/70 mb-1">After</p>
            <p className="text-xl font-bold text-yellow-400 drop-shadow-[0_0_10px_rgba(234,179,8,0.3)]">{after.toFixed(title === 'Input Delay' ? 1 : 0)}{unit}</p>
          </div>
        </div>
        <div className="mt-4 pt-4 border-t border-[#1a1a1a] flex justify-between items-center">
          <span className="text-xs text-neutral-500 font-medium">Improvement</span>
          <span className="text-xs font-bold text-green-400">+{improvement}%</span>
        </div>
      </div>
    );
  };
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-[#0f0f0f] text-white font-sans">
      ...
      {/* 1. Dashboard Preview (Major Proof Point) */}
      <DashboardPreview />

      {/* 2. Performance Comparison Cards (Secondary Proof Point) */}
      <PerformanceStats />
