"""WNBA board ingest + warehouse serializers for live frontend.

Builds a multi-player WNBA prop board from ESPN schedule → roster/leaders →
...
def _build_one_prop(
    ...
    *,
    ...
    team_id = f"wnba:team:{athlete.team_external_id}" if athlete.team_external_id else None
    player = upsert_player(db, athlete, team_id=team_id)
    gamelogs = providers.gamelog.fetch_gamelog("WNBA", athlete.external_id)
    for gl in gamelogs:
    ...
    if odds_provider is not None and not hasattr(odds_provider, "quote_for_prop"):
        try:
            live = [
                q
                for q in odds_provider.fetch_player_prop_odds("WNBA")  # type: ignore[union-attr]
    ...
    game_row = db.get(Game, f"wnba:game:{game.external_id}")
    prop = upsert_prop(
        db,
        league="WNBA",
    ...
    if not existing:
        existing = PropAnalytics(id=str(uuid.uuid4()), prop_id=prop.id, league="WNBA")
        db.add(existing)
...
def import_wnba_slate(
    ...
    *,
    ...
    games = providers.schedule.fetch_schedule("WNBA", date)
    for g in games:
        upsert_game(db, g)
    db.flush()
    ...
    for game in games[:max_games]:
        ...
        if providers.injuries:
            for inj in providers.injuries.fetch_injuries("WNBA", game.external_id):
...
def list_wnba_props_from_warehouse(db: Session) -> list[dict[str, Any]]:
    rows = (
        db.execute(
    ...
    for prop, analytics, player in rows:
        ...
        if tip is None:
            # Prefer last live odds capture (or analytics compute) — never invent "now" as tipoff
            stamp = (
...
def get_wnba_prop_detail(db: Session, prop_id: str) -> Optional[dict[str, Any]]:
    ...
    if not player and base.get("playerId"):
        player = db.execute(
            select(Player).where(
                ...
                | (Player.id == f"wnba:player:{base['playerId']}")
            )
        ).scalar_one_or_none()
...
def list_wnba_player_cards(db: Session) -> list[dict[str, Any]]:
    props = list_wnba_props_from_warehouse(db)
    seen: set[str] = set()
    cards = []
    for p in props:
        pid = p.get("playerId")
...
def get_wnba_player_profile(db: Session, player_key: str) -> Optional[dict[str, Any]]:
    ...
    board_props = list_wnba_props_from_warehouse(db)
