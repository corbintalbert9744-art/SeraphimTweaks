"""Recent-form and season-baseline factors."""
    ...
    def evaluate(self, ctx: PredictionContext) -> FactorResult:
        ...
        if len(vals) < 3:
            return FactorResult(
                ...
                weight=0.0,
                impact="neutral",
                detail="Insufficient season sample",
                available=False,
            )
        # Baseline is the projection anchor — adjustment 0; value carried in raw.
        baseline = mean(vals)
        return FactorResult(
            key=self.key,
            label=self.label,
            ...
            weight=0.35,
            impact="neutral",
            detail=f"Season mean {baseline:.1f} over {len(vals)} games (projection anchor)",
            raw={"baseline": baseline, "samples": len(vals), "stdev": pstdev(vals) if len(vals) > 1 else 0.0},
            available=True,
        )


class RecentFormFactor:
    """Blend L5 / L10 vs season mean — hot/cold recent form."""
    ...
    def evaluate(self, ctx: PredictionContext) -> FactorResult:
        ...
        # Weight recent windows; shrink toward season
        blended = 0.55 * l5 + 0.30 * l10 + 0.15 * season
        adj = blended - season
        # Cap extreme form spikes
        ...
        return FactorResult(
            ...
            detail=f"L5 {l5:.1f} · L10 {l10:.1f} vs season {season:.1f} → {adj:+.1f}",
...
class HomeAwayFactor:
    ...
    def evaluate(self, ctx: PredictionContext) -> FactorResult:
        ...
        adj = 0.5 * (side_m - season)  # partial credit
        return FactorResult(
            key=self.key,
            label=self.label,
            adjustment=adj,
